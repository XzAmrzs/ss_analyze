...
def store_user_flux(iter, col_name):
    ...
    try:
        r = Redis(host=database_driver_host, port=database_driver_port, db=0)
        dao_store_and_update(r, col_name, iter)
    except Exception as e:
...
def dao_store_and_update(db, col_name, iter):
    for record in iter:
        if col_name == 'user_app_stream_flux':
            ...
            dt = {"timestamp": timestamp, "app": app, "stream": stream, "flux": flux}
            db.hmset('uasf_' + user, dt)

        elif col_name == 'user_app_flux':
            (user, timestamp, app), flux = record
            dt = {"timestamp": timestamp, "app": app, "flux": flux}
            db.hmset('uaf_' + user, dt)

        else:  # if col_name == 'user_flux':
            (user, timestamp), flux = record
            dt = {"timestamp": timestamp, "flux": flux}
            db.hmset('uf_' + user, dt)
...
def create_context(brokers, topic):
    ...
    user_app_flux_counts = user_app_stream_flux_counts.map(get_user_app_flux).reduceByKey(lambda x, y: x + y)
    user_flux_counts = user_app_flux_counts.map(get_user_flux).reduceByKey(lambda x, y: x + y)

    # 数据输出保存(这里要注意Dstream->RDD->单个元素,要遍历三层才能获得单个元素)
    user_app_stream_flux_counts.foreachRDD(
        lambda rdd: rdd.foreachPartition(lambda x: store_user_flux(x, col_name='user_app_stream_flux')))
    user_app_flux_counts.foreachRDD(
        lambda rdd: rdd.foreachPartition(lambda x: store_user_flux(x, col_name='user_app_flux')))
    user_flux_counts.foreachRDD(
        lambda rdd: rdd.foreachPartition(lambda x: store_user_flux(x, col_name='user_flux')))
